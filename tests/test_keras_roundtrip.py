...
def _make_covariance_batch(batch_size: int, n_assets: int) -> tf.Tensor:
    """Generate a positive definite covariance batch for spectral tests."""
    raw = tf.random.normal((batch_size, n_assets, n_assets))
    return tf.matmul(raw, raw, transpose_b=True)
...
GET_CONFIG_CASES = [
    ...
    (
        CorrelationEigenTransformLayer(
            ...
            final_hidden_layer_sizes=(4,),
            final_hidden_activation="relu",
            output_type=("covariance", "correlation", "eigenvalues"),
            epsilon=1e-4,
            name="corr_cfg",
        ),
        {
...
def test_correlation_eigen_model_roundtrip_keras_preserves_outputs_and_names(tmp_path):
    ...
    x_corr = _make_covariance_batch(2, 6)
